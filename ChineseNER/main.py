...
def evaluate_line():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(TEXT_NUM):
            line_file = open("../Spider/civil_case/datastop/datastop_"+str(i)+"_cooked.txt","r")
            line = line_file.read()
            list_line = list(line)
            if len(line) > 2:
                #list_line = list(line)
                result = model.evaluate_line(sess,input_from_line(line,char_to_id),id_to_tag)
                outputs = open("../Spider/civil_case/datane/datane_"+str(i)+"_cooked.txt","w")
                for enti in result:
                    list_line[enti[0]:enti[1]] = ["" for i in range(enti[1]-enti[0])]
            outputs.write(''.join(list_line))
                #print(result)
                #line = input("请输入测试句子:")
                #result = model.evaluate_line(sess, input_from_line(line, char_to_id), id_to_tag)
                #print(result)
